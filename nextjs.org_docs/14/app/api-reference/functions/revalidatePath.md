...
[API Reference](/docs/14/app/api-reference)[Functions](/docs/14/app/api-reference/functions)revalidatePath

You are currently viewing documentation for version 14 of Next.js.

# revalidatePath

`revalidatePath` allows you to purge [cached data](/docs/14/app/building-your-application/caching) on-demand for a specific path.

> **Good to know**:
> 
> - `revalidatePath` is available in both [Node.js and Edge runtimes](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
> - `revalidatePath` only invalidates the cache when the included path is next visited. This means calling `revalidatePath` with a dynamic route segment will not immediately trigger many revalidations at once. The invalidation only happens when the path is next visited.
> - Currently, `revalidatePath` invalidates all the routes in the [client-side Router Cache](/docs/14/app/building-your-application/caching#router-cache). This behavior is temporary and will be updated in the future to apply only to the specific path.
> - Using `revalidatePath` invalidates **only the specific path** in the [server-side Route Cache](/docs/14/app/building-your-application/caching#full-route-cache).
