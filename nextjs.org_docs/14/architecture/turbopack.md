---
title: "Turbopack"
source: "https://nextjs.org/docs/14/architecture/turbopack"
---
...
[Introduction](/docs/14)[Architecture](/docs/14/architecture)Turbopack
...
Turbopack in Next.js requires zero-configuration for most users and can be extended for more advanced use cases. To learn more about the currently supported features for Turbopack, view the [API Reference](/docs/14/app/api-reference/next-config-js/turbo).
...
- [`webpack()`](/docs/14/app/api-reference/next-config-js/webpack) configuration in `next.config.js`
  
  - Turbopack replaces Webpack, this means that webpack configuration is not supported.
  - To configure Turbopack, [see the documentation](/docs/14/app/api-reference/next-config-js/turbo).
  - A subset of [Webpack loaders](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) are supported in Turbopack.
- Babel (`.babelrc`)
  
  - Turbopack leverages the [SWC](/docs/14/architecture/nextjs-compiler#why-swc) compiler for all transpilation and optimizations. This means that Babel is not included by default.
  - If you have a `.babelrc` file, you might no longer need it because Next.js includes common Babel plugins as SWC transforms that can be enabled. You can read more about this in the [compiler documentation](docs/14/architecture/nextjs-compiler#supported-features).
  - If you still need to use Babel after verifying your particular use case is not covered, you can leverage Turbopack's [support for custom webpack loaders](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) to include `babel-loader`.
...
- `@next/font` (legacy font support).
  ...
  - `@next/font` is deprecated in favor of `next/font`. [`next/font`](/docs/14/app/building-your-application/optimizing/fonts) is fully supported with Turbopack.
- `new Worker('file', import.meta.url)`.
  
  - We are planning to implement this in the future.
- [Relay transforms](/docs/14/architecture/nextjs-compiler#relay)
...
- [`experimental.urlImports`](/docs/14/app/api-reference/next-config-js/urlImports)
