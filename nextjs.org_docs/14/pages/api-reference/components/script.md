---
title: "<Script>"
source: "https://nextjs.org/docs/14/pages/api-reference/components/script"
---
...
[API Reference](/docs/14/pages/api-reference)[Components](/docs/14/pages/api-reference/components)&lt;Script&gt;

You are currently viewing the Pages Router documentation for version 14 of Next.js.

# &lt;Script&gt;

This API reference will help you understand how to use [props](#props) available for the Script Component. For features and usage, please see the [Optimizing Scripts](/docs/14/app/building-your-application/optimizing/scripts) page.
...
PropExampleTypeRequired[`src`](#src)`src="http://example.com/script"`StringRequired unless inline script is used[`strategy`](#strategy)`strategy="lazyOnload"`String-[`onLoad`](#onload)`onLoad={onLoadFunc}`Function-[`onReady`](#onready)`onReady={onReadyFunc}`Function-[`onError`](#onerror)`onError={onErrorFunc}`Function-
...
> **Warning:** The `worker` strategy is not yet stable and does not yet work with the [`app`](/docs/14/app/building-your-application/routing/defining-routes) directory. Use with caution.
...
VersionChanges`v13.0.0``beforeInteractive` and `afterInteractive` is modified to support `app`.`v12.2.4``onReady` prop added.`v12.2.2`Allow `next/script` with `beforeInteractive` to be placed in `_document`.`v11.0.0``next/script` introduced.
