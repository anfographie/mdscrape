---
title: "<Image> (Legacy)"
source: "https://nextjs.org/docs/14/pages/api-reference/components/image-legacy"
---
...
[API Reference](/docs/14/pages/api-reference)[Components](/docs/14/pages/api-reference/components)&lt;Image&gt; (Legacy)
...
View the **new** [`next/image` API Reference](/docs/14/pages/api-reference/components/image)
...
- A [statically imported](/docs/14/pages/building-your-application/optimizing/images#local-images) image file
...
The `width` property is required, except for [statically imported images](/docs/14/pages/building-your-application/optimizing/images#local-images), or those with `layout="fill"`.
...
The `height` property is required, except for [statically imported images](/docs/14/pages/building-your-application/optimizing/images#local-images), or those with `layout="fill"`.
...
When `blur`, the [`blurDataURL`](#blurdataurl) property will be used as the placeholder. If `src` is an object from a [static import](/docs/14/pages/building-your-application/optimizing/images#local-images) and the imported image is `.jpg`, `.png`, `.webp`, or `.avif`, then `blurDataURL` will be automatically populated.
...
You can configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/docs/14/pages/building-your-application/optimizing/images#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](/docs/14/pages/api-reference/next-config-js/headers) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
