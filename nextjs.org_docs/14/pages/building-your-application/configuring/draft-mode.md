---
title: "Draft Mode"
source: "https://nextjs.org/docs/14/pages/building-your-application/configuring/draft-mode"
---
...
[Building Your Application](/docs/14/pages/building-your-application)[Configuring](/docs/14/pages/building-your-application/configuring)Draft Mode
...
Then, send a request to `/api/disable-draft` to invoke the API Route. If calling this route using [`next/link`](/docs/14/pages/api-reference/components/link), you must pass `prefetch={false}` to prevent accidentally deleting the cookie on prefetch.

### [Works with `getServerSideProps`](#works-with-getserversideprops)

Draft Mode works with `getServerSideProps`, and is available as a `draftMode` key in the [`context`](/docs/14/pages/api-reference/functions/get-server-side-props#context-parameter) object.

> **Good to know**: You shouldn't set the `Cache-Control` header when using Draft Mode because it cannot be bypassed. Instead, we recommend using [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
