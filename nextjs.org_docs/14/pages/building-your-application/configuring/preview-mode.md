...
[Building Your Application](/docs/14/pages/building-your-application)[Configuring](/docs/14/pages/building-your-application/configuring)Preview Mode

You are currently viewing the Pages Router documentation for version 14 of Next.js.

# Preview Mode

> **Note**: This feature is superseded by [Draft Mode](/docs/14/pages/building-your-application/configuring/draft-mode).
...
In the [Pages documentation](/docs/14/pages/building-your-application/routing/pages-and-layouts) and the [Data Fetching documentation](/docs/14/pages/building-your-application/data-fetching), we talked about how to pre-render a page at build time (**Static Generation**) using `getStaticProps` and `getStaticPaths`.
...
> Take a look at the [API Routes documentation](/docs/14/pages/building-your-application/routing/api-routes) first if you’re not familiar with Next.js API Routes.
...
> **Good to know**: during rendering `next/router` exposes an `isPreview` flag, see the [router object docs](/docs/14/pages/api-reference/functions/use-router#router-object) for more info.
...
Then, send a request to `/api/clear-preview-mode-cookies` to invoke the API Route. If calling this route using [`next/link`](/docs/14/pages/api-reference/components/link), you must pass `prefetch={false}` to prevent calling `clearPreviewData` during link prefetching.
...
> **Good to know**: You shouldn't set the `Cache-Control` header when using Preview Mode because it cannot be bypassed. Instead, we recommend using [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
