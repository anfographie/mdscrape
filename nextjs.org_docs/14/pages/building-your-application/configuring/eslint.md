---
title: "ESLint"
source: "https://nextjs.org/docs/14/pages/building-your-application/configuring/eslint"
---
...
[Building Your Application](/docs/14/pages/building-your-application)[Configuring](/docs/14/pages/building-your-application/configuring)ESLint
...
> If you do not want ESLint to run during `next build`, refer to the documentation for [Ignoring ESLint](/docs/14/pages/api-reference/next-config-js/eslint).
...
RuleDescription[@next/next/google-font-display](/docs/14/messages/google-font-display)Enforce font-display behavior with Google Fonts.[@next/next/google-font-preconnect](/docs/14/messages/google-font-preconnect)Ensure `preconnect` is used with Google Fonts.[@next/next/inline-script-id](/docs/14/messages/inline-script-id)Enforce `id` attribute on `next/script` components with inline content.[@next/next/next-script-for-ga](/docs/14/messages/next-script-for-ga)Prefer `next/script` component when using the inline script for Google Analytics.[@next/next/no-assign-module-variable](/docs/14/messages/no-assign-module-variable)Prevent assignment to the `module` variable.[@next/next/no-async-client-component](/docs/14/messages/no-async-client-component)Prevent client components from being async functions.[@next/next/no-before-interactive-script-outside-document](/docs/14/messages/no-before-interactive-script-outside-document)Prevent usage of `next/script`'s `beforeInteractive` strategy outside of `pages/_document.js`.[@next/next/no-css-tags](/docs/14/messages/no-css-tags)Prevent manual stylesheet tags.[@next/next/no-document-import-in-page](/docs/14/messages/no-document-import-in-page)Prevent importing `next/document` outside of `pages/_document.js`.[@next/next/no-duplicate-head](/docs/14/messages/no-duplicate-head)Prevent duplicate usage of `<Head>` in `pages/_document.js`.[@next/next/no-head-element](/docs/14/messages/no-head-element)Prevent usage of `<head>` element.[@next/next/no-head-import-in-document](/docs/14/messages/no-head-import-in-document)Prevent usage of `next/head` in `pages/_document.js`.[@next/next/no-html-link-for-pages](/docs/14/messages/no-html-link-for-pages)Prevent usage of `<a>` elements to navigate to internal Next.js pages.[@next/next/no-img-element](/docs/14/messages/no-img-element)Prevent usage of `<img>` element due to slower LCP and higher bandwidth.[@next/next/no-page-custom-font](/docs/14/messages/no-page-custom-font)Prevent page-only custom fonts.[@next/next/no-script-component-in-head](/docs/14/messages/no-script-component-in-head)Prevent usage of `next/script` in `next/head` component.[@next/next/no-styled-jsx-in-document](/docs/14/messages/no-styled-jsx-in-document)Prevent usage of `styled-jsx` in `pages/_document.js`.[@next/next/no-sync-scripts](/docs/14/messages/no-sync-scripts)Prevent synchronous scripts.[@next/next/no-title-in-document-head](/docs/14/messages/no-title-in-document-head)Prevent usage of `<title>` with `Head` component from `next/document`.@next/next/no-typosPrevent common typos in [Next.js's data fetching functions](/docs/14/pages/building-your-application/data-fetching)[@next/next/no-unwanted-polyfillio](/docs/14/messages/no-unwanted-polyfillio)Prevent duplicate polyfills from Polyfill.io.
...
To improve performance, information of files processed by ESLint are cached by default. This is stored in `.next/cache` or in your defined [build directory](/docs/14/pages/api-reference/next-config-js/distDir). If you include any ESLint rules that depend on more than the contents of a single source file and need to disable the cache, use the `--no-cache` flag with `next lint`.
...
> The `next/core-web-vitals` entry point is automatically included for new applications built with [Create Next App](/docs/14/app/api-reference/cli/create-next-app).
...
- You have one or more of the following plugins already installed (either separately or through a different config such as `airbnb` or `react-app`):
  ...
  - `jsx-a11y`
  - `import`
- You've defined specific `parserOptions` that are different from how Babel is configured within Next.js (this is not recommended unless you have [customized your Babel configuration](/docs/14/pages/building-your-application/configuring/babel))
