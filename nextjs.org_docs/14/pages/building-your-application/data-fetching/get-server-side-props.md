...
source: "https://nextjs.org/docs/14/pages/building-your-application/data-fetching/get-server-side-props"
---
...
[Building Your Application](/docs/14/pages/building-your-application)[Data Fetching](/docs/14/pages/building-your-application/data-fetching)getServerSideProps
...
If you do not need to fetch the data at request time, or would prefer to cache the data and pre-rendered HTML, we recommend using [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props).
...
- When a user visits the page through [`next/link`](/docs/14/pages/api-reference/components/link) or [`next/router`](/docs/14/pages/api-reference/functions/use-router), Next.js sends an API request to the server, which runs `getServerSideProps`.
- You do not have to call a Next.js [API Route](/docs/14/pages/building-your-application/routing/api-routes) to fetch data when using `getServerSideProps` since the function runs on the server. Instead, you can call a CMS, database, or other third-party APIs directly from inside `getServerSideProps`.

> **Good to know:**
> 
> - See [`getServerSideProps` API reference](/docs/14/pages/api-reference/functions/get-server-side-props) for parameters and props that can be used with `getServerSideProps`.
> - You can use the [next-code-elimination tool](https://next-code-elimination.vercel.app/) to verify what Next.js eliminates from the client-side bundle.

## [Error Handling](#error-handling)

If an error is thrown inside `getServerSideProps`, it will show the `pages/500.js` file. Check out the documentation for [500 page](/docs/14/pages/building-your-application/routing/custom-error#500-page) to learn more on how to create it. During development, this file will not be used and the development error overlay will be shown instead.

## [Edge Cases](#edge-cases)

### [Edge Runtime](#edge-runtime)

`getServerSideProps` can be used with both [Serverless and Edge Runtimes](/docs/14/pages/building-your-application/rendering/edge-and-nodejs-runtimes), and you can set props in both.
...
However, before reaching for `cache-control`, we recommend seeing if [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) with [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) is a better fit for your use case.
