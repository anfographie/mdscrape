...
source: "https://nextjs.org/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration"
---
...
[Building Your Application](/docs/14/pages/building-your-application)[Data Fetching](/docs/14/pages/building-your-application/data-fetching)Incremental Static Regeneration (ISR)
...
> **Good to know**: The [`edge` runtime](/docs/14/pages/api-reference/edge) is currently not compatible with ISR, although you can leverage `stale-while-revalidate` by setting the `cache-control` header manually.
...
Next, add the secret as an [Environment Variable](/docs/14/pages/building-your-application/configuring/environment-variables) to your application. Finally, create the revalidation API Route:
...
When running locally with `next dev`, `getStaticProps` is invoked on every request. To verify your on-demand ISR configuration is correct, you will need to create a [production build](/docs/14/pages/api-reference/cli/next#next-build-options) and start the [production server](/docs/14/pages/api-reference/cli/next#next-start-options):
...
Incremental Static Regeneration (ISR) works on [self-hosted Next.js sites](/docs/14/pages/building-your-application/deploying#self-hosting) out of the box when you use `next start`.

Learn more about [self-hosting Next.js](/docs/14/pages/building-your-application/deploying#self-hosting).

## [Version History](#version-history)

VersionChanges`v14.1.0`Custom `cacheHandler` is stable.`v12.2.0`On-Demand ISR is stable`v12.1.0`On-Demand ISR added (beta).`v12.0.0`[Bot-aware ISR fallback](https://nextjs.org/blog/next-12#bot-aware-isr-fallback) added.`v9.5.0`Base Path added.
