...
source: "https://nextjs.org/docs/13/pages/building-your-application/data-fetching/get-server-side-props"
---
...
[Building Your Application](/docs/13/pages/building-your-application)[Data Fetching](/docs/13/pages/building-your-application/data-fetching)getServerSideProps
...
- When you request this page on client-side page transitions through [`next/link`](/docs/13/pages/api-reference/components/link) or [`next/router`](/docs/13/pages/api-reference/functions/use-router), Next.js sends an API request to the server, which runs `getServerSideProps`
...
The [`getServerSideProps` API reference](/docs/13/pages/api-reference/functions/get-server-side-props) covers all parameters and props that can be used with `getServerSideProps`.

## [When should I use getServerSideProps](#when-should-i-use-getserversideprops)

You should use `getServerSideProps` only if you need to render a page whose data must be fetched at request time. This could be due to the nature of the data or properties of the request (such as `authorization` headers or geo location). Pages using `getServerSideProps` will be server side rendered at request time and only be cached if [cache-control headers are configured](/docs/13/pages/building-your-application/deploying/production-checklist#caching).

If you do not need to render the data during the request, then you should consider fetching data on the [client side](#fetching-data-on-the-client-side) or [`getStaticProps`](/docs/13/pages/building-your-application/data-fetching/get-static-props).

### [getServerSideProps or API Routes](#getserversideprops-or-api-routes)

It can be tempting to reach for an [API Route](/docs/13/pages/building-your-application/routing/api-routes) when you want to fetch data from the server, then call that API route from `getServerSideProps`. This is an unnecessary and inefficient approach, as it will cause an extra request to be made due to both `getServerSideProps` and API Routes running on the server.

Take the following example. An API route is used to fetch some data from a CMS. That API route is then called directly from `getServerSideProps`. This produces an additional call, reducing performance. Instead, directly import the logic used inside your API Route into `getServerSideProps`. This could mean calling a CMS, database, or other API directly from inside `getServerSideProps`.

### [getServerSideProps with Edge API Routes](#getserversideprops-with-edge-api-routes)

`getServerSideProps` can be used with both [Serverless and Edge Runtimes](/docs/13/pages/building-your-application/rendering/edge-and-nodejs-runtimes), and you can set props in both. However, currently in the Edge Runtime, you do not have access to the response object. This means that you cannot — for example — add cookies in `getServerSideProps`. To have access to the response object, you should **continue to use the Node.js runtime**, which is the default runtime.
...
If your page contains frequently updating data, and you don’t need to pre-render the data, you can fetch the data on the [client side](/docs/13/pages/building-your-application/data-fetching/client-side). An example of this is user-specific data:
