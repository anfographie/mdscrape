...
source: "https://nextjs.org/docs/13/pages/building-your-application/routing/authenticating"
---
...
[Building Your Application](/docs/13/pages/building-your-application)[Routing](/docs/13/pages/building-your-application/routing)Authenticating
...
The first step to identifying which authentication pattern you need is understanding the [data-fetching strategy](/docs/13/pages/building-your-application/data-fetching) you want. We can then determine which authentication providers support this strategy. There are two main patterns:

- Use [static generation](/docs/13/pages/building-your-application/rendering/static-site-generation) to server-render a loading state, followed by fetching user data client-side.
- Fetch user data [server-side](/docs/13/pages/building-your-application/rendering/server-side-rendering) to eliminate a flash of unauthenticated content.

### [Authenticating Statically Generated Pages](#authenticating-statically-generated-pages)

Next.js automatically determines that a page is static if there are no blocking data requirements. This means the absence of [`getServerSideProps`](/docs/13/pages/building-your-application/data-fetching/get-server-side-props) and `getInitialProps` in the page. Instead, your page can render a loading state from the server, followed by fetching the user client-side.

One advantage of this pattern is it allows pages to be served from a global CDN and preloaded using [`next/link`](/docs/13/pages/api-reference/components/link). In practice, this results in a faster TTI ([Time to Interactive](https://web.dev/interactive/)).
...
If you export an `async` function called [`getServerSideProps`](/docs/13/pages/building-your-application/data-fetching/get-server-side-props) from a page, Next.js will pre-render this page on each request using the data returned by `getServerSideProps`.
