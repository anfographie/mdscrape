---
title: "useRouter"
source: "https://nextjs.org/docs/13/pages/api-reference/functions/use-router"
---
...
[API Reference](/docs/13/pages/api-reference)[Functions](/docs/13/pages/api-reference/functions)useRouter
...
- `query`: `Object` - The query string parsed to an object, including [dynamic route](/docs/13/pages/building-your-application/routing/dynamic-routes) parameters. It will be an empty object during prerendering if the page doesn't use [Server-side Rendering](/docs/13/pages/building-your-application/data-fetching/get-server-side-props). Defaults to `{}`
...
- `isFallback`: `boolean` - Whether the current page is in [fallback mode](/docs/13/pages/api-reference/functions/get-static-paths#fallback-true).
- `basePath`: `String` - The active [basePath](/docs/13/app/api-reference/next-config-js/basePath) (if enabled).
- `locale`: `String` - The active locale (if enabled).
- `locales`: `String[]` - All supported locales (if enabled).
- `defaultLocale`: `String` - The current default locale (if enabled).
- `domainLocales`: `Array<{domain, defaultLocale, locales}>` - Any configured domain locales.
- `isReady`: `boolean` - Whether the router fields are updated client-side and ready for use. Should only be used inside of `useEffect` methods and not for conditionally rendering on the server. See related docs for use case with [automatically statically optimized pages](/docs/13/pages/building-your-application/rendering/automatic-static-optimization)
- `isPreview`: `boolean` - Whether the application is currently in [preview mode](/docs/13/pages/building-your-application/configuring/preview-mode).

> Using the `asPath` field may lead to a mismatch between client and server if the page is rendered using server-side rendering or [automatic static optimization](/docs/13/pages/building-your-application/rendering/automatic-static-optimization). Avoid using `asPath` until the `isReady` field is `true`.
...
Handles client-side transitions, this method is useful for cases where [`next/link`](/docs/13/pages/api-reference/components/link) is not enough.
...
- `options` - Optional object with the following configuration options:
  ...
  - [`shallow`](/docs/13/pages/building-your-application/routing/linking-and-navigating#shallow-routing): Update the path of the current page without rerunning [`getStaticProps`](/docs/13/pages/building-your-application/data-fetching/get-static-props), [`getServerSideProps`](/docs/13/pages/building-your-application/data-fetching/get-server-side-props) or [`getInitialProps`](/docs/13/pages/api-reference/functions/get-initial-props). Defaults to `false`
...
Redirecting the user to `pages/login.js`, useful for pages behind [authentication](/docs/13/pages/building-your-application/routing/authenticating):
...
You can use a URL object in the same way you can use it for [`next/link`](/docs/13/pages/api-reference/components/link#with-url-object). Works for both the `url` and `as` parameters:
...
Similar to the `replace` prop in [`next/link`](/docs/13/pages/api-reference/components/link), `router.replace` will prevent adding a new URL entry into the `history` stack.
...
Prefetch pages for faster client-side transitions. This method is only useful for navigations without [`next/link`](/docs/13/pages/api-reference/components/link), as `next/link` takes care of prefetching pages automatically.
...
In some cases (for example, if using a [Custom Server](/docs/13/pages/building-your-application/configuring/custom-server)), you may wish to listen to [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) and do something before the router acts on it.
...
If `cb` returns `false`, the Next.js router will not handle `popstate`, and you'll be responsible for handling it in that case. See [Disabling file-system routing](/docs/13/pages/building-your-application/configuring/custom-server#disabling-file-system-routing).
...
> **Good to know**: Here `url` is the URL shown in the browser, including the [`basePath`](/docs/13/app/api-reference/next-config-js/basePath).
...
> We use a [Custom App](/docs/13/pages/building-your-application/routing/custom-app) (`pages/_app.js`) for this example to subscribe to the event because it's not unmounted on page navigations, but you can subscribe to router events on any component in your application.
