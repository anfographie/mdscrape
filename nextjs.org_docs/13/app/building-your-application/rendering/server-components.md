...
source: "https://nextjs.org/docs/13/app/building-your-application/rendering/server-components"
---
...
[Building Your Application](/docs/13/app/building-your-application)[Rendering](/docs/13/app/building-your-application/rendering)Server Components
...
By default, Next.js uses Server Components. This allows you to automatically implement server rendering with no additional configuration, and you can opt into using Client Components when needed, see [Client Components](/docs/13/app/building-your-application/rendering/client-components).
...
> 
> Learn more about the [full-route cache](/docs/13/app/building-your-application/caching#full-route-cache) and [Data Cache](/docs/13/app/building-your-application/caching#data-cache).

#### [Switching to Dynamic Rendering](#switching-to-dynamic-rendering)

During rendering, if a [dynamic function](#dynamic-functions) or [uncached data request](/docs/13/app/building-your-application/data-fetching/fetching-caching-and-revalidating#opting-out-of-data-caching) is discovered, Next.js will switch to dynamically rendering the whole route. This table summarizes how dynamic functions and data caching affect whether a route is statically or dynamically rendered:

Dynamic FunctionsDataRouteNoCachedStatically RenderedYesCachedDynamically RenderedNoNot CachedDynamically RenderedYesNot CachedDynamically Rendered
...
As a developer, you do not need to choose between static and dynamic rendering as Next.js will automatically choose the best rendering strategy for each route based on the features and APIs used. Instead, you choose when to [cache or revalidate specific data](/docs/13/app/building-your-application/data-fetching/fetching-caching-and-revalidating), and you may choose to [stream](#streaming) parts of your UI.

#### [Dynamic Functions](#dynamic-functions)

Dynamic functions rely on information that can only be known at request time such as a user's cookies, current requests headers, or the URL's search params. In Next.js, these dynamic functions are:

- **[`cookies()`](/docs/13/app/api-reference/functions/cookies) and [`headers()`](/docs/13/app/api-reference/functions/headers)**: Using these in a Server Component will opt the whole route into dynamic rendering at request time.
- [**`useSearchParams()`**](/docs/13/app/api-reference/functions/use-search-params):
  ...
  - We recommend wrapping the Client Component that uses `useSearchParams()` in a `<Suspense/>` boundary. This will allow any Client Components above it to be statically rendered. [Example](/docs/13/app/api-reference/functions/use-search-params#static-rendering).
- [**`searchParams`**](/docs/13/app/api-reference/file-conventions/page#searchparams-optional): Using the [Pages](/docs/13/app/api-reference/file-conventions/page) prop will opt the page into dynamic rendering at request time.

Using any of these functions will opt the whole route into dynamic rendering at request time.

### [Streaming](#streaming)

![Diagram showing parallelization of route segments during streaming, showing data fetching, rendering, and hydration of invidiual chunks.](/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Fdocs%2Flight%2Fsequential-parallel-data-fetching.png&w=3840&q=75)![Diagram showing parallelization of route segments during streaming, showing data fetching, rendering, and hydration of invidiual chunks.](/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Fdocs%2Fdark%2Fsequential-parallel-data-fetching.png&w=3840&q=75)
...
You can start streaming route segments using `loading.js` and UI components with [React Suspense](/docs/13/app/building-your-application/routing/loading-ui-and-streaming). See the [Loading UI and Streaming](/docs/13/app/building-your-application/routing/loading-ui-and-streaming) section for more information.
...
**Caching**  
\
An overview of caching mechanisms in Next.js.](/docs/13/app/building-your-application/caching)
