...
[Building Your Application](/docs/13/app/building-your-application)[Upgrading](/docs/13/app/building-your-application/upgrading)Migrating from Vite
...
2. **No automatic code splitting**: The previous issue of slow loading times can be somewhat managed with code splitting. However, if you try to do code splitting manually, you'll often make performance worse. It's easy to inadvertently introduce network waterfalls when code-splitting manually. Next.js provides automatic code splitting built into its router.
3. **Network waterfalls**: A common cause of poor performance occurs when applications make sequential client-server requests to fetch data. One common pattern for data fetching in an SPA is to initially render a placeholder, and then fetch data after the component has mounted. Unfortunately, this means that a child component that fetches data can't start fetching until the parent component has finished loading its own data. On Next.js, [this issue is resolved](https://github.com/reactjs/rfcs/blob/main/text/0188-server-components.md#no-client-server-waterfalls) by fetching data in Server Components.
4. **Fast and intentional loading states**: Thanks to built-in support for [Streaming with Suspense](/docs/13/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense), with Next.js, you can be more intentional about which parts of your UI you want to load first and in what order without introducing network waterfalls. This enables you to build pages that are faster to load and also eliminate [layout shifts](https://web.dev/cls/).
...
6. **Middleware**: [Next.js Middleware](/docs/13/app/building-your-application/routing/middleware) allows you to run code on the server before a request is completed. This is especially useful to avoid having a flash of unauthenticated content when the user visits an authenticated-only page by redirecting the user to a login page. The middleware is also useful for experimentation and internationalization.
7. **Built-in Optimizations**: Images, fonts, and third-party scripts often have significant impact on an application's performance. Next.js comes with built-in components that automatically optimize those for you.
...
Create a `next.config.mjs` at the root of your project. This file will hold your [Next.js configuration options](/docs/13/app/api-reference/next-config-js).
...
You can find more information about configuring TypeScript on the [Next.js docs](/docs/13/app/building-your-application/configuring/typescript#typescript-plugin).

### [Step 4: Create the Root Layout](#step-4-create-the-root-layout)

A Next.js [App Router](/docs/13/app) application must include a [root layout](/docs/13/app/building-your-application/routing/pages-and-layouts#root-layout-required) file, which is a [React Server Component](/docs/13/app/building-your-application/rendering/server-components) that will wrap all pages in your application. This file is defined at the top level of the `app` directory.
...
5. Any [metadata files](/docs/13/app/building-your-application/optimizing/metadata#file-based-metadata) such as `favicon.ico`, `icon.png`, `robots.txt` are automatically added to the application `<head>` tag as long as you have them placed into the top level of the `app` directory. After moving [all supported files](/docs/13/app/building-your-application/optimizing/metadata#file-based-metadata) into the `app` directory you can safely delete their `<link>` tags:
...
6. Finally, Next.js can manage your last `<head>` tags with the [Metadata API](/docs/13/app/building-your-application/optimizing/metadata). Move your final metadata info into an exported [`metadata` object](/docs/13/app/api-reference/functions/generate-metadata#metadata-object):
...
This directory is what is called an [optional catch-all route segment](/docs/13/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments). Next.js uses a file-system based router where [directories are used to define routes](/docs/13/app/building-your-application/routing/defining-routes#creating-routes). This special directory will make sure that all routes of your application will be directed to its containing `page.tsx` file.
...
This file contains a `<Page>` component which is marked as a [Client Component](/docs/13/app/building-your-application/rendering/client-components) by the `'use client'` directive. Without that directive, the component would have been a [Server Component](/docs/13/app/building-your-application/rendering/server-components).

In Next.js, Client Components are [prerendered to HTML](/docs/13/app/building-your-application/rendering/client-components#how-are-client-components-rendered) on the server before being sent to the client, but since we want to first have a purely client-side application, you need to tell Next.js to disable the prerendering for the `<App>` component by dynamically importing it with the `ssr` option set to `false`:
...
Next.js has support for `.env` [environment variables](/docs/13/app/building-your-application/configuring/environment-variables) similar to Vite. The main difference is the prefix used to expose environment variables on the client-side.
...
2. **Set [`basePath`](/docs/13/app/api-reference/next-config-js/basePath) to `process.env.NEXT_PUBLIC_BASE_PATH` in your `next.config.mjs` file:**
...
- Migrate from React Router to the [Next.js App Router](/docs/13/app/building-your-application/routing) to get:
  
  - Automatic code splitting
  - [Streaming Server-Rendering](/docs/13/app/building-your-application/routing/loading-ui-and-streaming)
  - [React Server Components](/docs/13/app/building-your-application/rendering/server-components)
- [Optimize images with the `<Image>` component](/docs/13/app/building-your-application/optimizing/images)
- [Optimize fonts with `next/font`](/docs/13/app/building-your-application/optimizing/fonts)
- [Optimize third-party scripts with the `<Script>` component](/docs/13/app/building-your-application/optimizing/scripts)
- [Update your ESLint configuration to support Next.js rules](/docs/13/app/building-your-application/configuring/eslint)
