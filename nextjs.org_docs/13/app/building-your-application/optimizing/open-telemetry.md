...
[Building Your Application](/docs/13/app/building-your-application)[Optimizing](/docs/13/app/building-your-application/optimizing)OpenTelemetry
...
Next, create a custom [`instrumentation.ts`](/docs/13/app/building-your-application/optimizing/instrumentation) (or `.js`) file in the **root directory** of the project (or inside `src` folder if using one):
...
> - If you use the [`pageExtensions` config option](/docs/13/app/api-reference/next-config-js/pageExtensions) to add a suffix, you will also need to update the `instrumentation` filename to match.
