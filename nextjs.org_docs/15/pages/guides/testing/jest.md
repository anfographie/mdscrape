...
source: "https://nextjs.org/docs/15/pages/guides/testing/jest"
---
...
[Guides](/docs/15/pages/guides)[Testing](/docs/15/pages/guides/testing)Jest
...
- Setting up `transform` using the [Next.js Compiler](/docs/15/architecture/nextjs-compiler).
- Auto mocking stylesheets (`.css`, `.module.css`, and their scss variants), image imports and [`next/font`](/docs/15/app/api-reference/components/font).
- Loading `.env` (and all variants) into `process.env`.
- Ignoring `node_modules` from test resolving and transforms.
- Ignoring `.next` from test resolving.
- Loading `next.config.js` for flags that enable SWC transforms.

> **Good to know**: To test environment variables directly, load them manually in a separate setup script or in your `jest.config.ts` file. For more information, please see [Test Environment Variables](/docs/15/app/guides/environment-variables#test-environment-variables).

## [Setting up Jest (with Babel)](#setting-up-jest-with-babel)

If you opt out of the [Next.js Compiler](/docs/15/architecture/nextjs-compiler) and use Babel instead, you will need to manually configure Jest and install `babel-jest` and `identity-obj-proxy` in addition to the packages above.
...
If your project is using [Module Path Aliases](/docs/15/app/getting-started/installation#set-up-absolute-imports-and-module-path-aliases), you will need to configure Jest to resolve the imports by matching the paths option in the `jsconfig.json` file with the `moduleNameMapper` option in the `jest.config.js` file. For example:
