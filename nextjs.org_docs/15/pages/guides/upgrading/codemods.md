---
title: "Codemods"
source: "https://nextjs.org/docs/15/pages/guides/upgrading/codemods"
---
...
[Guides](/docs/15/pages/guides)[Upgrading](/docs/15/pages/guides/upgrading)Codemods
...
APIs that opted into dynamic rendering that previously supported synchronous access are now asynchronous. You can read more about this breaking change in the [upgrade guide](/docs/15/app/guides/upgrading/version-15).
...
This codemod moves transforms imports from `next/server` to `next/og` for usage of [Dynamic OG Image Generation](/docs/15/app/getting-started/metadata-and-og-images#generated-open-graph-images).
...
Remove `<a>` tags inside [Link Components](/docs/15/pages/api-reference/components/link), or add a `legacyBehavior` prop to Links that cannot be auto-fixed.
...
Transforms the deprecated automatically injected `url` property on top level pages to using `withRouter` and the `router` property it injects. Read more here: [https://nextjs.org/docs/messages/url-deprecated](/docs/15/messages/url-deprecated)
