---
title: "Image"
source: "https://nextjs.org/docs/15/pages/api-reference/components/image"
---
...
[API Reference](/docs/15/pages/api-reference)[Components](/docs/15/pages/api-reference/components)Image
...
> **Good to know**: If you are using a version of Next.js prior to 13, you'll want to use the [next/legacy/image](/docs/15/pages/api-reference/components/image-legacy) documentation since the component was renamed.
...
> **Good to know**: For security reasons, the Image Optimization API using the default [loader](#loader) will *not* forward headers when fetching the `src` image. If the `src` image requires authentication, consider using the [unoptimized](#unoptimized) property to disable Image Optimization.
...
- [Custom Image Loader Configuration](/docs/15/app/api-reference/config/next-config-js/images#example-loader-configuration)
...
`minimumCacheTTL` allows you to configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/docs/15/app/getting-started/images#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](/docs/15/app/api-reference/config/next-config-js/headers) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
...
- SVG has many of the same features as HTML/CSS, which can lead to vulnerabilities without proper [Content Security Policy (CSP) headers](/docs/15/app/api-reference/config/next-config-js/headers#content-security-policy).
...
Use `className` or `style`, not `styled-jsx`. In most cases, we recommend using the `className` prop. This can be an imported [CSS Module](/docs/15/app/getting-started/css), a [global stylesheet](/docs/15/app/getting-started/css#global-css), etc.
...
You cannot use [styled-jsx](/docs/15/app/guides/css-in-js) because it's scoped to the current component (unless you mark the style as `global`).
...
Since Next.js does not have access to remote files during the build process, you'll need to provide the [`width`](/docs/15/app/api-reference/components/image#width-and-height), [`height`](/docs/15/app/api-reference/components/image#width-and-height) and optional [`blurDataURL`](/docs/15/app/api-reference/components/image#blurdataurl) props manually.
