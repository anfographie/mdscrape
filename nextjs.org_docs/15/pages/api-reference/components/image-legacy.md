---
title: "Image (Legacy)"
source: "https://nextjs.org/docs/15/pages/api-reference/components/image-legacy"
---
...
[API Reference](/docs/15/pages/api-reference)[Components](/docs/15/pages/api-reference/components)Image (Legacy)
...
View the **new** [`next/image` API Reference](/docs/15/pages/api-reference/components/image)
...
- A [statically imported](/docs/15/pages/api-reference/components/image#src) image file
...
The `width` property is required, except for [statically imported images](/docs/15/pages/api-reference/components/image#src), or those with `layout="fill"`.
...
The `height` property is required, except for [statically imported images](/docs/15/pages/api-reference/components/image#src), or those with `layout="fill"`.
...
When `blur`, the [`blurDataURL`](#blurdataurl) property will be used as the placeholder. If `src` is an object from a [static import](/docs/15/pages/api-reference/components/image#src) and the imported image is `.jpg`, `.png`, `.webp`, or `.avif`, then `blurDataURL` will be automatically populated.
...
> Images can not be optimized at build time using [`output: 'export'`](/docs/15/pages/guides/static-exports), only on-demand. To use `next/legacy/image` with `output: 'export'`, you will need to use a different loader than the default. [Read more in the discussion.](https://github.com/vercel/next.js/discussions/19065)
...
You can configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/docs/15/pages/api-reference/components/image#src) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](/docs/15/pages/api-reference/config/next-config-js/headers) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
...
The default [loader](#loader) does not optimize SVG images for a few reasons. First, SVG is a vector format meaning it can be resized losslessly. Second, SVG has many of the same features as HTML/CSS, which can lead to vulnerabilities without proper [Content Security Policy (CSP) headers](/docs/15/app/api-reference/config/next-config-js/headers#content-security-policy).
...
VersionChanges`v13.0.0``next/image` renamed to `next/legacy/image`
