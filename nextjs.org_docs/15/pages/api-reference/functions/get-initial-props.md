...
source: "https://nextjs.org/docs/15/pages/api-reference/functions/get-initial-props"
---
...
[API Reference](/docs/15/pages/api-reference)[Functions](/docs/15/pages/api-reference/functions)getInitialProps

You are currently viewing the Pages Router documentation for version 15 of Next.js.

# getInitialProps

> **Good to know**: `getInitialProps` is a legacy API. We recommend using [`getStaticProps`](/docs/15/pages/building-your-application/data-fetching/get-static-props) or [`getServerSideProps`](/docs/15/pages/building-your-application/data-fetching/get-server-side-props) instead.
...
> - For the initial page load, `getInitialProps` will run on the server only. `getInitialProps` will then also run on the client when navigating to a different route with the [`next/link`](/docs/15/pages/api-reference/components/link) component or by using [`next/router`](/docs/15/pages/api-reference/functions/use-router).
...
- `getInitialProps` can only be used in `pages/` top level files, and not in nested components. To have nested data fetching at the component level, consider exploring the [App Router](/docs/15/app/getting-started/fetching-data).
